...
function mp_fb_poster_setting_page() {
    ...
    if (isset($_GET['fblogout']) && $_GET['fblogout'] == 1) {
        ...
        update_option('mp-getdeal-page-access-token', '');
        //setcookie("fbs_" . get_option('mp-getdeal-fb-app-key'), '', time()-3600, "/", "www.getdeals.nl");				
        $facebook->destroySession();
    } elseif (isset($_GET['state']) || (isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true')) {
    ...
    <div class="wrap">
        ...
        <form id="origin_setting" method="post" action="options.php">
            ...
            if (!get_option('mp-getdeal-userid') || !get_option('mp-getdeal-access-token')) {
                $params = array('scope' => 'manage_pages,publish_actions', 'redirect_uri' => get_option('home') . '/wp-admin/options-general.php?page=facebook-deal-poster-options');
                ?>
                ...
                $logoutUrl = get_option('home').'/wp-admin/options-general.php?page=facebook-deal-poster-options&fblogout=1';
